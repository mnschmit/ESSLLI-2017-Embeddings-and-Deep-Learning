...
class MR(TarDataset):
    ...
    def __init__(self, text_field, label_field,
        ...
        if examples is None:
            ...
            examples = []
            with codecs.open(
                    os.path.join(path, 'rt-polarity.neg'), 'r', 'iso-8859-15'
            ) as f:
                examples += [
                    data.Example.fromlist(
                        [line, 'negative'], fields
                    ) for line in f]
            with codecs.open(
                    os.path.join(path, 'rt-polarity.pos'), 'r', 'iso-8859-15'
            ) as f:
                examples += [
                    data.Example.fromlist(
                        [line, 'positive'], fields
                    ) for line in f]
        super(MR, self).__init__(examples, fields, **kwargs)

    @classmethod
    def splits(cls, text_field, label_field,
               dev_ratio=.1, shuffle=True, root='.', seed=42, **kwargs):
        """Create dataset objects for splits of the MR dataset.
        Arguments:
            text_field: The field that will be used for the sentence.
            label_field: The field that will be used for label data.
            dev_ratio: The ratio will be used to get split validation dataset.
